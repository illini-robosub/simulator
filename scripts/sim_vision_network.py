...
def point_to_pixel(point_array, lens_params=lens_parameters):
    ...
    # point.
    theta = np.arctan2(math.sqrt(point.x * point.x + point.y * point.y),
                       -point.z)
...
def change_reference_frame(point, rot_quat, origin):
    ...
    # Apply an empirically determined transformation to relocate the -Z axis
    # along
...
def camera_callback(image):
    ...
    for box in boxes:
        ...
        points.append([int((origin[0] + 1) / 2 * width),
                       int((1 - (origin[1] + 1) / 2) * height)])
        for location in pixels:
            location[1] /= ratio_scale_y
            points.append([int((location[0] + 1)/2 * width),
                           int((1 - (location[1] + 1) / 2) * height)])
        ...
        new_locations = cv2.fisheye.undistortPoints(np_array, K, D, np.eye(3),
                                                    K).reshape(-1, 2)

        point = new_locations[0].flatten()
        if debug:
            cv2.circle(undistorted, (int(point[0]), int(point[1])), 5,
                       (0, 0, 255), 2)
...
if __name__ == '__main__':
    ...
    for link in all_links:
        links[link['name']] = LinkDescriptor(link['label'], link['width'],
                                             link['height'], link['depth'],
                                             link['label_id'])

    # Start subscribes and publishers for the ROS framework.
    detection_pub = rospy.Publisher('vision/{}'.format(camera), DetectionArray,
                                    queue_size=1)
    rospy.Subscriber('gazebo/link_states', LinkStates, callback, queue_size=1)
    rospy.Subscriber('camera/{}/image_raw'.format(camera), Image,
                     camera_callback, queue_size=1)
