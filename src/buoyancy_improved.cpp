...
void BuoyancyPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
    ...
    physics::ModelPtr ceiling = _model->GetWorld()->ModelByName(
                                                              "ceiling_plane");
    if (ceiling) {
...
void BuoyancyPlugin::OnUpdate() {
    ignition::math::Vector3<double> net_buoyancy(0.0, 0.0, 0.0);
    for (auto link : this->model->GetLinks()) {
        ...
        // Therefore,
        ignition::math::Vector3<double> buoyancy =
            -this->fluidDensity * volume * link->GetWorld()->Gravity();

        ignition::math::Pose3<double> linkFrame = link->WorldPose();

        // rotate buoyancy into the link frame before applying the force.
        ignition::math::Vector3<double> buoyancyLinkFrame =
            linkFrame.Rot().Inverse().RotateVector(buoyancy);

        net_buoyancy += buoyancy;

        ignition::math::Vector3<double> absCov = linkFrame.Pos() +
                                                 volumeProperties.cov;
